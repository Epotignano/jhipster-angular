...
module app.services {
  ...
  export class StudentsService implements IStudentsService {
      ...
      this.FirebaseCRUDFactory.get(studentId, this.collectionKey)
        .then((studentData) => {
          ...
          studentData.courses.forEach((course)=> {
            var coursePromise = this.$q.defer();

            this.FirebaseCRUDFactory.get(course.id, 'courses')
              .then((courseData) => {
                courseData['id'] = course.id;
                console.log(courseData);
                coursePromise.resolve(courseData);
              })
              .catch((error) => {
                  console.log('TODO');
              });
            coursesPromises.push(coursePromise.promise);
          });

          this.$q.all(coursesPromises).then((results) => {
              console.log(results);
    ...
    removeStudentCourse(studentId, studentCourseId) {
        this.FirebaseCRUDFactory.remove(studentCourseId, this.collectionKey + '/' + studentId + '/courses');
     }
