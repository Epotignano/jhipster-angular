  ...
  export class CoursesService implements ICoursesService {
    ...
    update(courseObj, courseId) {
      this.FirebaseCRUDFactory.update(courseObj, courseId, this.collectionKey)
        .then((data)=> {
            this.thread.onNext({ data, 'EVENT': this.threadsService.defaultEvents.OBJECT_UPDATE });
        });
    }
    ...
    removeBookCourse(courseId, courseBookId) {
        this.FirebaseCRUDFactory.remove(courseId, this.collectionKey + '/' + courseBookId + '/books');
    }
  }
